...
```{r}
mcc.amount.pertrans <- mcc.feq %>% left_join(mcc.amount, by='MCCcode')
...
```{r}
MCC.Feq.All <- MCC.Feq.All %>%
  mutate(RiskRatio = MCCcodeFeqFPD/(MCCcodeFeqGood+MCCcodeFeqFPD) )
...
```{r}
MCC.Amount.All <- MCC.Amount.All %>%
  mutate(RiskRatio = MCCcodeAmountFPD/(MCCcodeAmountGood+MCCcodeAmountFPD) )
